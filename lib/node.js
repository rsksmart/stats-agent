...
Node.prototype.validateLatestBlock = function (error, result, timeString)
{
	...
	block.netHashRate = this.stats.nethashrate;
...
Node.prototype.getStats = function(forced)
{
	...
	if (this._web3 && (lastFetchAgo >= UPDATE_INTERVAL || forced === true))
	{
		...
		async.parallel({
			...
			nethashrate: function(callback)
			{
				web3.eth.netHashrate(callback);
			}
		},
		function (err, results)
		{
			...
			if(results.peers !== null)
			{
				...
				var hash = results.nethashrate.toString();
				if(hash !== "0.000" && hash !== "0" && hash !== "0.0" ) {
					self.lastNetHashRate = results.nethashrate;
				}
				self.stats.nethashrate = self.lastNetHashRate;
...
Node.prototype.prepareStats = function ()
{
	return {
		...
		stats: {
			...
			nethashrate: this.stats.nethashrate
		}
	};
}
